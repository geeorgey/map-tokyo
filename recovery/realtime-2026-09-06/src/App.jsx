import { React, ReactHooks, ReactDOM, TrainDialog, VIEWPOINTS } from '../vendor/scene-runtime.js';
...
import '../vendor/deployed/assets/index-ByugdiWY.css';
import './style.css';

const { useState, useRef, useEffect, useMemo } = ReactHooks;
...
function ClockPanel({ now, onChange, data, context }) {
  ...
  function apply(event) {
    ...
    const value = parseJapanDateTime(date, time);
  ...
  return <section className="clock-panel glass" aria-label="日本時間とシミュレーション時計">
    ...
    {editing && <form id="date-editor" className="date-editor" onSubmit={apply}>
      <label>日付<input aria-label="再現する日付" type="date" value={date} required onChange={event => setDate(event.target.value)} /></label>
      <label>時刻（日本時間）<input aria-label="再現する時刻" type="time" step="1" value={time} required onChange={event => setTime(event.target.value)} /></label>
...
function App() {
  ...
  useEffect(() => { engine.current?.setPeriod(period); }, [period, ready]);
  ...
  return <main className={`app ${period}`}>
    ...
    <div className="top-controls"><div className="period glass" role="group" aria-label="時間帯">{[['day', '昼'], ['evening', '夕'], ['night', '夜']].map(([value, label]) => <button key={value} aria-pressed={period === value} onClick={() => setPeriod(value)}>{label}</button>)}</div><button className="capture glass icon" onClick={saveScene} disabled={!ready} aria-label="風景をPNGで保存" title="風景をPNGで保存">↧</button></div>
