...
Search Console の所有権は HTML タグで確認済み。
サイトマップ送信は成功し、1 ページを検出。GA4 ウェブストリームとのリンクも作成済み。
公開ページから `page_view` が 1 回送信され HTTP 204 が返り、GA4 リアルタイムに
アクティブユーザー 1、ページタイトル「東京鉄道景 — Tokyo Railway Diorama」が表示された。
